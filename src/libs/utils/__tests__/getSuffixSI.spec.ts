...
describe('getSuffixSI', () => {
    ...
    it('should return an empty string with invalid numbers', () => {
        [null, undefined, '', -5, -10].forEach(number => {
            expect(getSuffixSI(number)).toBe('');
        });
    });

    it('should return the same string if it is not a valid number', () => {
        ['1k', '2k', '3k', '99+'].forEach(number => {
            expect(getSuffixSI(number)).toBe(number);
        });
    });
});
